-- Corrigir função de devolução de tokens para considerar custom_goal
...
DECLARE
  ...
  effective_goal NUMERIC;
BEGIN
  -- Usar custom_goal se existir, senão usar goal
  effective_goal := COALESCE(OLD.custom_goal, OLD.goal);
  
  -- Verificar se o projeto atingiu 100% da meta
  project_completed := (OLD.raised_amount >= effective_goal);
...
-- Corrigir função de cancelamento para considerar custom_goal
...
BEGIN
  ...
  IF NEW.status = 'cancelled' AND OLD.status != 'cancelled' THEN
    -- Usar custom_goal se existir, senão usar goal
    effective_goal := COALESCE(NEW.custom_goal, NEW.goal);
    
    -- Verificar se o projeto atingiu 100% da meta
    project_completed := (NEW.raised_amount >= effective_goal);
